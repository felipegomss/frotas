...
import { Vehicle, type VehicleRepository } from '@frotas/domain';
...
export class PrismaVehicleRepository implements VehicleRepository {
  ...
  listAvailable(): Promise<Vehicle[]> {
    return this.withTenant(async (tx) => {
      const rows = await tx.$queryRawUnsafe<VehicleRow[]>(
        `SELECT id, plate, status, current_mileage
           FROM vehicles
  ...
  save(vehicle: Vehicle): Promise<void> {
    return this.withTenant(async (tx) => {
      await tx.$executeRaw`
        INSERT INTO vehicles (id, plate, status, current_mileage)
        VALUES (${vehicle.id}, ${vehicle.plate}, ${vehicle.status}, ${vehicle.currentMileage})
