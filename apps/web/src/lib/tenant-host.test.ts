...
describe("tenantSlugFromHost", () => {
  const base = "amparofrota.com.br";

  it("extracts the slug from <slug>.<base-domain>", () => {
    expect(tenantSlugFromHost("demo.amparofrota.com.br", base)).toBe("demo");
    expect(tenantSlugFromHost("santa-rita.amparofrota.com.br", base)).toBe(
      "santa-rita",
    );
  });

  it("ignores the port", () => {
    expect(tenantSlugFromHost("demo.amparofrota.com.br:443", base)).toBe(
      "demo",
    );
  ...
  it("returns null for the bare domain or bare localhost", () => {
    expect(tenantSlugFromHost("amparofrota.com.br", base)).toBeNull();
  ...
  it("does not blocklist infra subdomains — they just won't match a membership", () => {
    ...
    expect(tenantSlugFromHost("www.amparofrota.com.br", base)).toBe("www");
  });

  it("returns null for nested or unrelated hosts", () => {
    expect(tenantSlugFromHost("a.b.amparofrota.com.br", base)).toBeNull();
